#include "GraphicsItemDrawMainWindow.h"
#include "ui_GraphicsItemDrawMainWindow.h"
...
GraphicsItemDrawMainWindow::GraphicsItemDrawMainWindow(QWidget *parent) :
    QMainWindow(parent),
    ui(new Ui::GraphicsItemDrawMainWindow)
{
    ui->setupUi(this);
...
GraphicsItemDrawMainWindow::~GraphicsItemDrawMainWindow()
{
    delete ui;
}

void GraphicsItemDrawMainWindow::on_mouseMovePoint(QPoint point)
...
void GraphicsItemDrawMainWindow::on_mouseClicked(QPoint point)
{//鼠标单击事件
    QPointF pointScene=ui->View->mapToScene(point); //转换到Scene坐标
...
void GraphicsItemDrawMainWindow::on_mouseDoubleClick(QPoint point)
...
void GraphicsItemDrawMainWindow::on_keyPress(QKeyEvent *event)
{ //按键事件
...
void GraphicsItemDrawMainWindow::on_actItem_Rect_triggered()
{ //添加一个矩形
...
void GraphicsItemDrawMainWindow::on_actItem_Ellipse_triggered()
{ //添加一个椭圆
...
void GraphicsItemDrawMainWindow::on_actItem_Polygon_triggered()
{ //添加一个梯形
...
void GraphicsItemDrawMainWindow::on_actEdit_Delete_triggered()
{ //删除所有选中的绘图项
...
void GraphicsItemDrawMainWindow::on_actZoomIn_triggered()
{ //放大
    int cnt=scene->selectedItems().count();
    if (cnt==1)
    {
...
void GraphicsItemDrawMainWindow::on_actZoomOut_triggered()
{//缩小
    int cnt=scene->selectedItems().count();
    if (cnt==1)
    {
...
void GraphicsItemDrawMainWindow::on_actRestore_triggered()
{//取消所有变换
    int cnt=scene->selectedItems().count();
    if (cnt==1)
    {
...
void GraphicsItemDrawMainWindow::on_actRotateLeft_triggered()
{//逆时针旋转
    int cnt=scene->selectedItems().count();
    if (cnt==1)
    {
...
void GraphicsItemDrawMainWindow::on_actRotateRight_triggered()
{//顺时针旋转
    int cnt=scene->selectedItems().count();
    if (cnt==1)
    {
...
void GraphicsItemDrawMainWindow::on_actEdit_Front_triggered()
{ //bring to front,前置
    int cnt=scene->selectedItems().count();
    if (cnt>0)
    { //只处理选中的第1个绘图项
...
void GraphicsItemDrawMainWindow::on_actEdit_Back_triggered()
{//bring to back，后置
    int cnt=scene->selectedItems().count();
    if (cnt>0)
    {//只处理选中的第1个绘图项
...
void GraphicsItemDrawMainWindow::on_actItem_Line_triggered()
{//添加直线
...
void GraphicsItemDrawMainWindow::on_actItem_Text_triggered()
{ //添加文字
...
void GraphicsItemDrawMainWindow::on_actGroup_triggered()
{ //组合
    int cnt=scene->selectedItems().count();
    if (cnt>1)
    {
...
void GraphicsItemDrawMainWindow::on_actGroupBreak_triggered()
{ //break group,打散组合
    int cnt=scene->selectedItems().count();
    if (cnt==1)
    {
...
void GraphicsItemDrawMainWindow::on_actItem_Circle_triggered()
{ //添加圆形
...
void GraphicsItemDrawMainWindow::on_actItem_Triangle_triggered()
{ //添加三角形
    QGraphicsPolygonItem   *item=new QGraphicsPolygonItem;
    QPolygonF   points;

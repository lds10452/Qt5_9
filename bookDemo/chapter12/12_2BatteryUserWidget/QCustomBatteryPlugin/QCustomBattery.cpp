...
void QCustomBattery::paintEvent(QPaintEvent *event)
{  //界面组件的绘制
    Q_UNUSED(event);
...
QCustomBattery::QCustomBattery(QWidget *parent) :
    QWidget(parent)
{
}

void QCustomBattery::setPowerLevel(int pow)
{ //设置当前电量值
    mPowerLevel=pow;
...
int QCustomBattery::powerLevel()
{ //读取当前电量值
    return mPowerLevel;
}

void QCustomBattery::setWarnLevel(int warn)
{//设置电量低阈值
    mWarnLevel=warn;
    repaint();
}

int QCustomBattery::warnLevel()
{//读取电量低阈值
    return  mWarnLevel;
}

QSize QCustomBattery::sizeHint()
{//报告缺省大小,调整比例
    int H=this->height();
    int W=H*12/5;
    QSize   size(W,H);

#include "CustomDialogsMainWindow.h"
#include "ui_CustomDialogsMainWindow.h"
#include    <QMessageBox>
#include    <QCloseEvent>

#include    "qwdialogsize.h"

void CustomDialogsMainWindow::closeEvent(QCloseEvent *event)
{ //窗口关闭时询问是否退出
...
CustomDialogsMainWindow::CustomDialogsMainWindow(QWidget *parent) :
    QMainWindow(parent),
    ui(new Ui::CustomDialogsMainWindow)
{
    ui->setupUi(this);
...
CustomDialogsMainWindow::~CustomDialogsMainWindow()
{
    delete ui;
}

void CustomDialogsMainWindow::setActLocateEnable(bool enable)
{
    ui->actTab_Locate->setEnabled(enable);
}

void CustomDialogsMainWindow::setACellText(int row, int column, QString text)
{//定位到单元格，并设置字符串
...
void CustomDialogsMainWindow::setDlgLocateNull()
{
    dlgLocate=NULL;
}


void CustomDialogsMainWindow::on_currentChanged(const QModelIndex &current, const QModelIndex &previous)
{
...
void CustomDialogsMainWindow::on_actTab_SetSize_triggered()
{ //模态对话框，动态创建，用过后删除
...
void CustomDialogsMainWindow::on_actTab_SetHeader_triggered()
...
void CustomDialogsMainWindow::on_actTab_Locate_triggered()
...
void CustomDialogsMainWindow::on_tableView_clicked(const QModelIndex &index)

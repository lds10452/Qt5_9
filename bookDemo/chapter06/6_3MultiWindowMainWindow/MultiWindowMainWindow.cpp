#include "MultiWindowMainWindow.h"
#include "ui_MultiWindowMainWindow.h"
#include    <QPainter>
#include    <QFileDialog>
//#include    "qwmisc.h"
void MultiWindowMainWindow::paintEvent(QPaintEvent *event)
{ //绘制窗口背景图片
    Q_UNUSED(event);
    QPainter painter(this);
    painter.drawPixmap(0,ui->mainToolBar->height(),this->width(),
                       this->height()-ui->mainToolBar->height()-ui->statusBar->height(),
                       QPixmap(":/res/images/back2.jpg"));
}

MultiWindowMainWindow::MultiWindowMainWindow(QWidget *parent) :
    QMainWindow(parent),
    ui(new Ui::MultiWindowMainWindow)
{
    ui->setupUi(this);
...
MultiWindowMainWindow::~MultiWindowMainWindow()
{
    delete ui;
}

void MultiWindowMainWindow::on_tabWidget_tabCloseRequested(int index)
{//关闭Tab
    if (index<0)
        return;
...
void MultiWindowMainWindow::on_actWidgetInsite_triggered()
...
void MultiWindowMainWindow::on_actWindowInsite_triggered()
{
...
void MultiWindowMainWindow::on_actWindow_triggered()
{
    QFormTable* formTable = new QFormTable(this);
...
void MultiWindowMainWindow::on_actWidget_triggered()
{
...
void MultiWindowMainWindow::on_tabWidget_currentChanged(int index)
{
    Q_UNUSED(index);

#include "QStringListModelWidget.h"
#include "ui_QStringListModelWidget.h"

QStringListModelWidget::QStringListModelWidget(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::QStringListModelWidget)
{
    ui->setupUi(this);
...
QStringListModelWidget::~QStringListModelWidget()
{
    delete ui;
}

void QStringListModelWidget::on_listView_clicked(const QModelIndex &index)
{ //显示QModelIndex的行、列号
    ui->LabInfo->setText(QString::asprintf("当前项:row=%d, column=%d",
                        index.row(),index.column()));
}

void QStringListModelWidget::on_btnIniList_clicked()
...
void QStringListModelWidget::on_btnTextClear_clicked()
{ //清除plainTextEdit的文本
    ui->plainTextEdit->clear();
}

void QStringListModelWidget::on_btnTextImport_clicked()
{// 显示数据模型的StringList
    QStringList tmpList;
...
void QStringListModelWidget::on_btnListClear_clicked()
{//清除ListView的所有项
    theModel->removeRows(0,theModel->rowCount());
}

void QStringListModelWidget::on_btnListAppend_clicked()
{ //添加一行
...
void QStringListModelWidget::on_btnListInsert_clicked()
{//插入一行
    QModelIndex  index;
...
void QStringListModelWidget::on_btnListDelete_clicked()
{//删除当前行
    QModelIndex  index;

#include "MultiLangMainWindow.h"
#include "ui_MultiLangMainWindow.h"
...
#include    "../../chapter06/6_2CustomDialogsMainWindow/qwdialogsize.h"

extern QTranslator *trans;

void MultiLangMainWindow::closeEvent(QCloseEvent *event)
{ //窗口关闭时询问是否退出
...
MultiLangMainWindow::MultiLangMainWindow(QWidget *parent) :
    QMainWindow(parent),
    ui(new Ui::MultiLangMainWindow)
{
    ui->setupUi(this);
...
MultiLangMainWindow::~MultiLangMainWindow()
{
    delete ui;
}

void MultiLangMainWindow::setActLocateEnable(bool enable)
{ //设置actTab_Locate的enable属性
    ui->actTab_Locate->setEnabled(enable);
}

void MultiLangMainWindow::selectACell(int row, int column)
{
...
void MultiLangMainWindow::setACellText(int row, int column, QString &text)
{//定位到单元格，并设置字符串
...
void MultiLangMainWindow::on_currentChanged(const QModelIndex &current, const QModelIndex &previous)
{
    Q_UNUSED(previous)
...
void MultiLangMainWindow::on_actTab_SetSize_triggered()
{ //模态对话框，动态创建，用过后删除
...
void MultiLangMainWindow::on_actTab_SetHeader_triggered()
...
void MultiLangMainWindow::on_actTab_Locate_triggered()
...
void MultiLangMainWindow::on_tableView_clicked(const QModelIndex &index)
{//单击单元格时，将单元格的行号、列号设置到对话框上
    emit cellIndexChanged(index.row(),index.column());
}

void MultiLangMainWindow::on_actLang_CN_triggered()
{//中文界面
    ...
    QSettings   settings("WWB-Qt","5_9"); //注册表键组
    settings.setValue("Language","CN"); //界面语言，汉语
}

void MultiLangMainWindow::on_actLang_EN_triggered()
{//英文界面
    qApp->removeTranslator(trans);
    delete trans;

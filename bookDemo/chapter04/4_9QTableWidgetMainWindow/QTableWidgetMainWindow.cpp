#include "QTableWidgetMainWindow.h"
#include "ui_QTableWidgetMainWindow.h"
...
void QTableWidgetMainWindow::createItemsARow(int rowNo,QString Name,QString Sex,QDate birth,QString Nation,bool isPM,int score)
{ //为一行的单元格创建 Items
...
//姓名
    ...
    item=new  QTableWidgetItem(Name,QTableWidgetMainWindow::ctName);
    ...
    ui->tableInfo->setItem(rowNo,QTableWidgetMainWindow::colName,item); //为单元格设置Item

//性别
    QIcon   icon;
    if (Sex=="男")
        icon.addFile(":/res/images/boy.ico");
    else
        icon.addFile(":/res/images/girl.ico");
    item=new  QTableWidgetItem(Sex,QTableWidgetMainWindow::ctSex); //新建一个Item，设置单元格type为自定义的 QTableWidgetMainWindow::ctSex
    item->setIcon(icon);
    item->setTextAlignment(Qt::AlignHCenter | Qt::AlignVCenter);//为单元格设置Item
    ui->tableInfo->setItem(rowNo,QTableWidgetMainWindow::colSex,item);//为单元格设置Item

//出生日期
    str=birth.toString("yyyy-MM-dd"); //日期转换为字符串
    item=new  QTableWidgetItem(str,QTableWidgetMainWindow::ctBirth);//新建一个Item，设置单元格type为自定义的 QTableWidgetMainWindow::ctBirth
    item->setTextAlignment(Qt::AlignLeft | Qt::AlignVCenter); //文本对齐格式
    ui->tableInfo->setItem(rowNo,QTableWidgetMainWindow::colBirth,item);//为单元格设置Item

//民族
    item=new  QTableWidgetItem(Nation,QTableWidgetMainWindow::ctNation); //新建一个Item，设置单元格type为自定义的 QTableWidgetMainWindow::ctNation
    item->setTextAlignment(Qt::AlignHCenter | Qt::AlignVCenter);//文本对齐格式
    ui->tableInfo->setItem(rowNo,QTableWidgetMainWindow::colNation,item);//为单元格设置Item

//是否党员
    item=new  QTableWidgetItem("党员",QTableWidgetMainWindow::ctPartyM);//新建一个Item，设置单元格type为自定义的 QTableWidgetMainWindow::ctPartyM
    item->setTextAlignment(Qt::AlignHCenter | Qt::AlignVCenter);//文本对齐格式
    if (isPM)
    ...
    ui->tableInfo->setItem(rowNo,QTableWidgetMainWindow::colPartyM,item);//为单元格设置Item

//分数
    str.setNum(score);
    item=new  QTableWidgetItem(str,QTableWidgetMainWindow::ctScore);//新建一个Item，设置单元格type为自定义的 QTableWidgetMainWindow::ctPartyM
    item->setTextAlignment(Qt::AlignHCenter | Qt::AlignVCenter);//文本对齐格式
    ui->tableInfo->setItem(rowNo,QTableWidgetMainWindow::colScore,item);//为单元格设置Item
}

QTableWidgetMainWindow::QTableWidgetMainWindow(QWidget *parent) :
    QMainWindow(parent),
    ui(new Ui::QTableWidgetMainWindow)
{
    ui->setupUi(this);
    //状态栏初始化创建
...
QTableWidgetMainWindow::~QTableWidgetMainWindow()
{
    delete ui;
}

void QTableWidgetMainWindow::on_btnSetHeader_clicked()
{ //设置表头
...
void QTableWidgetMainWindow::on_btnSetRows_clicked()
...
void QTableWidgetMainWindow::on_btnIniData_clicked()
{ //初始化表格内容
...
void QTableWidgetMainWindow::on_chkBoxTabEditable_clicked(bool checked)
{ //设置编辑模式
    if (checked)
//双击或获取焦点后单击，进入编辑状态
...
void QTableWidgetMainWindow::on_chkBoxHeaderH_clicked(bool checked)
{//是否显示水平表头
    ui->tableInfo->horizontalHeader()->setVisible(checked);
}

void QTableWidgetMainWindow::on_chkBoxHeaderV_clicked(bool checked)
{//是否显示垂直表头
    ui->tableInfo->verticalHeader()->setVisible(checked);
}

void QTableWidgetMainWindow::on_chkBoxRowColor_clicked(bool checked)
{ //行的底色交替采用不同颜色
    ui->tableInfo->setAlternatingRowColors(checked);
}

void QTableWidgetMainWindow::on_rBtnSelectItem_clicked()
{//选择行为：单元格选择
    ui->tableInfo->setSelectionBehavior(QAbstractItemView::SelectItems);
}

void QTableWidgetMainWindow::on_rBtnSelectRow_clicked()
{//选择行为：行选择
    ui->tableInfo->setSelectionBehavior(QAbstractItemView::SelectRows);
}

void QTableWidgetMainWindow::on_btnReadToEdit_clicked()
...
void QTableWidgetMainWindow::on_tableInfo_currentCellChanged(int currentRow, int currentColumn, int previousRow, int previousColumn)
{//当前选择单元格发生变化时的响应
   ...
   Q_UNUSED(previousColumn);
    ...
    item=ui->tableInfo->item(currentRow,QTableWidgetMainWindow::colName); //取当前行第1列的单元格的 item
    int ID=item->data(Qt::UserRole).toInt();//读取用户自定义数据
    labStudID->setText(QString::asprintf("学生ID：%d",ID));//学生ID
}


void QTableWidgetMainWindow::on_btnInsertRow_clicked()
{ //插入一行
...
void QTableWidgetMainWindow::on_btnAppendRow_clicked()
{ //添加一行
...
void QTableWidgetMainWindow::on_btnDelCurRow_clicked()
{//删除当前行及其items
    int curRow=ui->tableInfo->currentRow();//当前行号
    ui->tableInfo->removeRow(curRow); //删除当前行及其items
}

void QTableWidgetMainWindow::on_btnAutoHeght_clicked()
{
    ui->tableInfo->resizeRowsToContents();
}

void QTableWidgetMainWindow::on_btnAutoWidth_clicked()
{

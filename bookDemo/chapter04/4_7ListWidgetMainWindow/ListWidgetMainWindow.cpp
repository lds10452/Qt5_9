...
void ListWidgetMainWindow::setActionsForButton()
...
void ListWidgetMainWindow::createSelectionPopMenu()
{//创建下拉菜单
...
ListWidgetMainWindow::ListWidgetMainWindow(QWidget *parent) :
    QMainWindow(parent),
    ui(new Ui::ListWidgetMainWindow)
{
    ui->setupUi(this);
    setActionsForButton();
...
void ListWidgetMainWindow::on_actListIni_triggered()
{//初始化项
        QListWidgetItem *aItem; //每一行是一个QListWidgetItem

        QIcon aIcon;
        aIcon.addFile(":/res/images/check2.ico"); //设置ICON的图标
...
void ListWidgetMainWindow::on_actListInsert_triggered()
{//插入一个项
    QIcon aIcon;
    aIcon.addFile(":/res/images/check2.ico"); //图标
...
void ListWidgetMainWindow::on_actListDelete_triggered()
{//删除当前项
...
void ListWidgetMainWindow::on_actSelALL_triggered()
{//项全选
...
void ListWidgetMainWindow::on_actListClear_triggered()
{
    ui->listWidget->clear(); //清除项
}

void ListWidgetMainWindow::on_chkBoxListEditable_clicked(bool checked)
...
void ListWidgetMainWindow::on_actListAppend_triggered()
{//增加一个项
    QIcon aIcon;
    aIcon.addFile(":/res/images/check2.ico"); //设定图标
...
void ListWidgetMainWindow::on_toolBox_currentChanged(int index)
{
    ui->tabWidget->setCurrentIndex(index);//ToolBox当前页与tabWidget的当前页联动
}


void ListWidgetMainWindow::on_actSelNone_triggered()
{ //全不选
    int i,cnt;
    QListWidgetItem *aItem;
...
void ListWidgetMainWindow::on_actSelInvs_triggered()
{//反选
    int i,cnt;
    QListWidgetItem *aItem;

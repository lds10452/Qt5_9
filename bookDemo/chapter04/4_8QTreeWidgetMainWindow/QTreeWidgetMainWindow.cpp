...
void QTreeWidgetMainWindow::iniTree()
{ //初始化Tree
    QString    dataStr=""; // Item的Data 存储的string

    ui->treeFiles->clear();//清除目录树所有节点
    QIcon   icon;
    icon.addFile(":/res/images/15.ico"); //设置ICON的图标

    QTreeWidgetItem*  item=new QTreeWidgetItem(QTreeWidgetMainWindow::itTopItem); //新建节点时设定类型为 itTopItem
    item->setIcon(QTreeWidgetMainWindow::colItem,icon); //设置第1列的图标
    item->setText(QTreeWidgetMainWindow::colItem,"图片文件"); //设置第1列的文字
    item->setText(QTreeWidgetMainWindow::colItemType,"type=itTopItem");  //设置第2列的文字
    item->setFlags(Qt::ItemIsSelectable | Qt::ItemIsUserCheckable | Qt::ItemIsEnabled | Qt::ItemIsAutoTristate);
    item->setCheckState(colItem,Qt::Checked);//设置为选中

    item->setData(QTreeWidgetMainWindow::colItem,Qt::UserRole,QVariant(dataStr)); //设置节点第1列的Qt::UserRole的Data
    ui->treeFiles->addTopLevelItem(item);//添加顶层节点
}

void QTreeWidgetMainWindow::addFolderItem(QTreeWidgetItem *parItem, QString dirName)
{//添加一个目录节点
    QIcon   icon(":/res/images/open3.bmp");
    QString NodeText=getFinalFolderName(dirName); //从一个完整目录名称里，获得最后的文件夹名称

    QTreeWidgetItem *item; //节点
    item=new QTreeWidgetItem(QTreeWidgetMainWindow::itGroupItem); //新建节点, 设定type为 itGroupItem
...
QString QTreeWidgetMainWindow::getFinalFolderName(const QString &fullPathName)
...
void QTreeWidgetMainWindow::addImageItem(QTreeWidgetItem *parItem, QString aFilename)
{//添加一个图片文件节点
    QIcon   icon(":/res/images/31.ico");//ICON的图标
    QString NodeText=getFinalFolderName(aFilename); //获得最后的文件名称

    QTreeWidgetItem *item; //节点
    item=new QTreeWidgetItem(QTreeWidgetMainWindow::itImageItem); //新建节点时设定类型为 itImageItem
    item->setIcon(colItem,icon); //设置图标
...
void QTreeWidgetMainWindow::displayImage(QTreeWidgetItem *item)
{//显示图片,节点item存储的图片文件名
...
void QTreeWidgetMainWindow::changeItemCaption(QTreeWidgetItem *item)
{ //改变节点的标题文字
...
void QTreeWidgetMainWindow::on_treeFiles_currentItemChanged(QTreeWidgetItem *current, QTreeWidgetItem *previous)
{ //当前节点选择变化时触发
    Q_UNUSED(previous);
    if  (current==NULL)
        return;
...
void QTreeWidgetMainWindow::on_actAddFolder_triggered()
...
void QTreeWidgetMainWindow::on_actAddFiles_triggered()
{//添加图片文件节点
...
void QTreeWidgetMainWindow::on_actZoomOut_triggered()
{ //缩小显示
...
void QTreeWidgetMainWindow::on_actZoomIn_triggered()
{//放大显示
...
void QTreeWidgetMainWindow::on_actZoomFitW_triggered()
{ //适应宽度显示
...
void QTreeWidgetMainWindow::on_actZoomFitH_triggered()
{//适应高度显示图片
...
void QTreeWidgetMainWindow::on_actZoomRealSize_triggered()
{ //实际大小显示
    pixRatio=1;  //恢复显示比例为1
    ui->LabPicture->setPixmap(curPixmap);
}

void QTreeWidgetMainWindow::on_actDeleteItem_triggered()
{//删除节点
...
void QTreeWidgetMainWindow::on_actScanItems_triggered()
{//遍历节点
...
void QTreeWidgetMainWindow::on_actDockVisible_toggled(bool arg1)
{// 停靠区的可见性
    ui->dockWidget->setVisible(arg1);
}

void QTreeWidgetMainWindow::on_dockWidget_visibilityChanged(bool visible)
{//停靠区可见性变化
    ui->actDockVisible->setChecked(visible);
}

void QTreeWidgetMainWindow::on_dockWidget_topLevelChanged(bool topLevel)
{//停靠区浮动性变化
    ui->actDockFloat->setChecked(topLevel);
}

void QTreeWidgetMainWindow::on_actDockFloat_triggered(bool checked)
{//停靠区浮动性

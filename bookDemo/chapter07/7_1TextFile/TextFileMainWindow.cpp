#include "TextFileMainWindow.h"
#include "ui_TextFileMainWindow.h"
...
bool TextFileMainWindow::openTextByIODevice(const QString &aFileName)
{//用IODevice方式打开文本文件
...
bool TextFileMainWindow::openTextByStream(const QString &aFileName)
{ //用 QTextStream打开文本文件
    QFile   aFile(aFileName);
...
bool TextFileMainWindow::saveTextByIODevice(const QString &aFileName)
{ //用IODevice方式保存文本文件
    QFile   aFile(aFileName);
//    aFile.setFileName(aFileName);
...
bool TextFileMainWindow::saveTextByStream(const QString &aFileName)
{//用QTextStream保存文本文件
    QFile   aFile(aFileName);
...
TextFileMainWindow::TextFileMainWindow(QWidget *parent) :
    QMainWindow(parent),
    ui(new Ui::TextFileMainWindow)
{
    ui->setupUi(this);
}

TextFileMainWindow::~TextFileMainWindow()
{
    delete ui;
}

void TextFileMainWindow::on_actOpen_IODevice_triggered()
{//打开文件
...
void TextFileMainWindow::on_actOpen_TextStream_triggered()
{ //打开文件
    QString curPath=QDir::currentPath();//获取系统当前目录
//调用打开文件对话框打开一个文件
...
void TextFileMainWindow::on_actSave_IODevice_triggered()
{
    QString curPath=QDir::currentPath();//获取系统当前目录
...
void TextFileMainWindow::on_actSave_TextStream_triggered()
{
    QString curPath=QDir::currentPath();//获取系统当前目录
    QString dlgTitle="另存为一个文件"; //对话框标题
    QString filter="h文件(*.h);;c++文件(*.cpp);;文本文件(*.txt);;所有文件(*.*)"; //文件过滤器

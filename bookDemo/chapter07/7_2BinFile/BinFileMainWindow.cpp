#include "BinFileMainWindow.h"
#include "ui_BinFileMainWindow.h"

#include    <QFileDialog>
#include    <QDataStream>
#include    <QMessageBox>

void BinFileMainWindow::resetTable(int aRowCount)
...
bool BinFileMainWindow::saveDataAsStream(QString &aFileName)
...
bool BinFileMainWindow::openDataAsStream(QString &aFileName)
{ //从Qt预定义流文件读入数据
    QFile aFile(aFileName);  //以文件方式读出
...
bool BinFileMainWindow::saveBinaryFile(QString &aFileName)
{ //保存为纯二进制文件
    QFile aFile(aFileName);  //以文件方式读出
...
bool BinFileMainWindow::openBinaryFile(QString &aFileName)
{//打开二进制文件
    QFile aFile(aFileName);  //以文件方式读出
    if (!(aFile.open(QIODevice::ReadOnly)))
        return false;
...
BinFileMainWindow::BinFileMainWindow(QWidget *parent) :
    QMainWindow(parent),
    ui(new Ui::BinFileMainWindow)
{
    ui->setupUi(this);
...
BinFileMainWindow::~BinFileMainWindow()
{
    delete ui;
}

void BinFileMainWindow::on_currentChanged(const QModelIndex &current, const QModelIndex &previous)
{
    Q_UNUSED(previous);
    if (current.isValid())
    {
...
void BinFileMainWindow::on_actOpen_triggered()
{
...
void BinFileMainWindow::on_actAppend_triggered()
{ //添加行
...
void BinFileMainWindow::on_actInsert_triggered()
{//插入行
...
void BinFileMainWindow::on_actDelete_triggered()
{ //删除行
    QModelIndex curIndex=theSelection->currentIndex();
...
void BinFileMainWindow::on_actSave_triggered()
{ //以Qt预定义编码保存数据文件
    QString curPath=QDir::currentPath();
...
void BinFileMainWindow::on_actAlignCenter_triggered()
{
...
void BinFileMainWindow::on_actFontBold_triggered(bool checked)
{
    if (!theSelection->hasSelection())
        return;
...
void BinFileMainWindow::on_actAlignLeft_triggered()
{
    if (!theSelection->hasSelection())
        return;
...
void BinFileMainWindow::on_actAlignRight_triggered()
{
    if (!theSelection->hasSelection())
        return;
...
void BinFileMainWindow::on_actTabReset_triggered()
{//表格复位
    resetTable(10);
}

void BinFileMainWindow::on_actSaveBin_triggered()
{//保存二进制文件
    QString curPath=QDir::currentPath();
    //调用打开文件对话框选择一个文件
    QString aFileName=QFileDialog::getSaveFileName(this,tr("选择保存文件"),curPath,
...
void BinFileMainWindow::on_actOpenBin_triggered()
{//打开二进制文件
